...
WORD CalcCRC16(const void* data, UINT size)
{
	...
	for (UINT i = 0; i < size; ++i) {
		crc16 = (crc16 << 8) ^ CRC16_TABLE[ ptr[i] ^ unsigned char(crc16 >> 8)];
	}
...
UINT CalcCRC32(const void* data, UINT size)
{
	...
	const BYTE* ptr = static_cast<const BYTE*>(data);
	for (UINT i = 0; i < size; ++i) {
		crc32 = (crc32 >> 8) ^ CRC32_TABLE[ ptr[i] ^ unsigned char(crc32 & 0xFF)];
	}
...
UINT CalcFNV32a(const void* data, UINT size)
{
	...
	for (UINT i = 0; i < size; ++i) {
		hashcode ^= static_cast<UINT>(ptr[i]);
...
UINT64 CalcFNV64a(const void* data, UINT size)
{
	...
	for (UINT i = 0; i < size; ++i) {
		hashcode ^= static_cast<UINT64>(ptr[i]);
		hashcode += (hashcode << 1) + (hashcode << 4) + (hashcode << 5) + (hashcode << 7) + (hashcode << 8) + (hashcode << 24);
	}
